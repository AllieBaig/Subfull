...
export const SystemIntegrityView = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'critical': return 'text-system-red';
      default: return 'text-system-gray';
    }
  };

  const getIcon = (val: boolean) => {
    return val ? <CheckCircle2 size={14} className="text-system-green" /> : <AlertCircle size={14} className="text-system-red" />;
  };

  if (!report) return null;

  return (
    <div className="bg-tertiary-system-background rounded-[2rem] border border-apple-border shadow-sm overflow-hidden mb-8">
      <button 
            ...
            <div className="p-6 space-y-4">
              ...
              {report.errors.length > 0 && (
                <div className="flex flex-col gap-2">
                  <p className="text-[9px] font-black text-system-red uppercase tracking-widest pl-1">Diagnostic Logs</p>
                  <div className="bg-system-red/5 border border-system-red/10 rounded-2xl p-4 space-y-2">
                    {report.errors.map((err, i) => (
                      <div key={i} className="flex gap-2 text-[10px] font-medium text-system-red leading-tight">
              ...
              <div className="flex flex-col gap-2">
                 <div className="flex items-center gap-2 px-1">
                    ...
                    <span className="text-[9px] font-black text-system-blue uppercase tracking-widest">Health Monitor</span>
                 </div>
