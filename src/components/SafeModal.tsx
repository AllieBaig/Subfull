      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-sm bg-tertiary-system-background rounded-[2.5rem] shadow-2xl border border-apple-border overflow-hidden pointer-events-auto"
          >
    ...
    <SafeModal isOpen={isOpen} onClose={onClose} title={title} subtitle={subtitle}>
      <div className="flex flex-col gap-6">
        <input 
          ...
          className="w-full bg-secondary-system-background text-system-label rounded-2xl p-5 text-lg font-bold outline-none border border-apple-border focus:ring-0 focus:border-system-blue shadow-inner-sm transition-all"
          autoFocus
          spellCheck={false}
          autoComplete="off"
        />
        ...
        <div className="flex gap-3 mt-2">
          ...
          <button 
            ...
            disabled={!value.trim()}
            className="flex-[1.5] py-4.5 rounded-2xl bg-system-blue text-system-background font-bold uppercase text-[11px] tracking-widest transition-all active:scale-[0.98] disabled:opacity-30 shadow-lg shadow-system-blue/20"
          >
            {confirmLabel}
          </button>
        </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex gap-3">
          ...
          <button 
            ...
            className={`flex-1 py-4.5 rounded-2xl ${isDestructive ? 'bg-system-red shadow-system-red/20' : 'bg-system-label shadow-system-label/20'} text-system-background font-bold uppercase text-[11px] tracking-widest transition-all active:scale-[0.98] shadow-lg`}
          >
            {confirmLabel}
          </button>
        </div>
