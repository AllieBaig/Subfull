  ...
  return (
    <div className="bg-system-background rounded-2xl border border-apple-border p-4 mt-4">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-8 h-8 rounded-xl bg-system-blue/10 text-system-blue flex items-center justify-center">
          <HardDrive size={16} />
        </div>
        <div>
      ...
      <div className="w-full h-1.5 bg-secondary-system-background rounded-full overflow-hidden">
        <div 
          className="h-full bg-system-blue rounded-full transition-all duration-1000"
          style={{ width: `${Math.min(100, ((usage || 0) / (quota || 1)) * 100)}%` }}
        />
      </div>

      <div className="flex items-start gap-2 mt-4 text-system-tertiary-label">
