...
import { AdvancedAudioIntelligence } from '../components/AdvancedAudioIntelligence';
import { AutoFeatureOrganizer } from '../components/AutoFeatureOrganizer';
import { HistoryImportExport } from '../components/HistoryImportExport';
          ...
          {isExpanded && (
            ...
            >
              <div className="p-6 flex flex-col gap-8">
                {/* Import & Export Tools */}
                <div className="bg-system-background rounded-3xl border border-apple-border p-5 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-[10px] font-black uppercase text-system-secondary-label tracking-widest pl-1">Import & Export</h4>
                    <span className="w-1.5 h-1.5 rounded-full bg-system-blue" />
                  </div>
                  <HistoryImportExport />
                </div>

                <div className="h-px bg-apple-border/20 mx-4" />
                ...
                {settings.versionHistory.map((entry, idx) => (
                  <div key={entry.version} className="relative pl-6 border-l border-apple-border last:border-0 pb-2">
                    <div className="absolute left-[-5px] top-1.5 w-2.5 h-2.5 rounded-full bg-system-blue shadow-[0_0_0_4px_var(--system-background)]" />
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      {idx === 0 && <span className="text-[8px] font-black bg-system-blue text-system-background px-1.5 py-0.5 rounded uppercase tracking-tighter">New</span>}
                    </div>
                    
                    <div className="flex flex-col gap-4">
                      {entry.changes.added && entry.changes.added.length > 0 && (
                        <div>
                          <p className="text-[9px] font-black text-system-green uppercase tracking-widest mb-1.5">Added</p>
                      ...
                      {entry.changes.improved && entry.changes.improved.length > 0 && (
                        <div>
                          <p className="text-[9px] font-black text-system-blue uppercase tracking-widest mb-1.5">Improved</p>
                          <ul className="space-y-1">
                      ...
                      {entry.changes.fixed && entry.changes.fixed.length > 0 && (
                        <div>
                          <p className="text-[9px] font-black text-system-orange uppercase tracking-widest mb-1.5">Fixed</p>
                          <ul className="space-y-1">
  ...
  return (
    <div className="flex flex-col pb-12 w-full max-w-7xl mx-auto">
      ...
      <Group 
        ...
        isExpanded={expandedGroups.has('intelligence')}
        onToggle={() => toggleGroup('intelligence')}
      >
        <div className="flex flex-col gap-2 p-1">
          <AdvancedAudioIntelligence />
        </div>
      </Group>
      ...
      <Group 
        ...
        isExpanded={expandedGroups.has('feature_intel')}
        onToggle={() => toggleGroup('feature_intel')}
      >
        <div className="flex flex-col gap-2 p-1">
          <Section
            id="auto_organizer"
            ...
            icon={FolderTree}
            color="bg-system-orange/10 text-system-orange"
            isEnabled={settings.autoOrganizer.isEnabled}
            onToggle={(v: boolean) => updateAutoOrganizerSettings({ isEnabled: v })}
          >
            <AutoFeatureOrganizer />
          </Section>
        </div>
      </Group>
      ...
      <Group 
        ...
        isExpanded={expandedGroups.has('app_intelligence')}
        onToggle={() => toggleGroup('app_intelligence')}
      >
        <div className="flex flex-col gap-2 p-1">
          <AppIntelligence />
        </div>
      ...
      <Group 
        title="App Control"
        icon={SettingsIcon}
        color="bg-system-tertiary-label/10 text-system-secondary-label"
          ...
          {swSupported && (
            ...
            >
              <div className="flex flex-col gap-3">
                ...
                <button onClick={fullAppReset} className="w-full p-4 bg-system-red text-system-background font-bold text-xs uppercase rounded-xl hover:bg-system-red/90 active:scale-[0.98] transition-all">Full Factory Reset</button>
              </div>
            </Section>
          )}

          <div className="bg-system-blue/5 p-4 rounded-2xl border border-system-blue/10 flex items-center justify-between mt-2">
            <div>
              <div className="flex items-center gap-1.5">
                <Activity size={10} className="text-system-blue" />
                <p className="text-[13px] font-extrabold text-system-blue">System Stabilization</p>
              </div>
            ...
            <button 
              onClick={healSystem}
              className="bg-system-blue text-system-background py-1.5 px-4 rounded-full text-[10px] font-black uppercase tracking-widest shadow-md active:scale-95 transition-transform"
            >
              Heal
            </button>
          </div>
