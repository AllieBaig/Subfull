      ...
      <header className="px-6 pt-10 pb-4 bg-system-background">
        ...
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full h-14 bg-secondary-system-background border-none pl-12 pr-12 py-4 rounded-2xl text-base font-medium outline-none transition-all placeholder:text-system-tertiary-label text-system-label focus:ring-2 focus:ring-system-blue/20"
          />
          {searchQuery && (
            <button 
            ...
            {filteredTracks.map((track) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 library-card rounded-2xl transition-all group ${isActive ? 'bg-system-blue/5' : 'bg-secondary-system-background/30 active:bg-secondary-system-background'}`}
                >
                      ...
                      {isActive && (
                        <div className="absolute inset-0 bg-system-blue/20 flex items-center justify-center">
                          <div className="flex gap-0.5 items-end h-2.5">
                            <div className="w-0.5 bg-system-blue animate-bounce" style={{ animationDelay: '0ms' }} />
                            <div className="w-0.5 bg-system-blue animate-bounce" style={{ animationDelay: '200ms' }} />
                            <div className="w-0.5 bg-system-blue animate-bounce" style={{ animationDelay: '400ms' }} />
                          </div>
                        </div>
                      )}
                    </div>
                    <div className="flex flex-col items-start min-w-0 text-left">
                      <span className={`text-[13px] font-bold truncate w-full ${isActive ? 'text-system-blue' : 'text-system-label'}`}>{track.name}</span>
                  ...
                  <ChevronRight size={16} className={`${isActive ? 'text-system-blue' : 'text-system-tertiary-label'} opacity-30`} />
                </button>
              );
            })}
          </div>
