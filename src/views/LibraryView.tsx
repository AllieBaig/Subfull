...
export default function LibraryView() {
  ...
  const controls = (
    <div className={cn(
      ...
      settings.libraryControlsPosition === 'bottom' && cn(
        "px-4 py-3 bg-system-background/80 backdrop-blur-2xl border-t border-apple-border fixed bottom-[83px] left-0 right-0 z-[110] safe-area-bottom shadow-[0_-10px_30px_rgba(0,0,0,0.03)]",
        isMini && "py-2 bottom-[75px]"
      )
    )}>
      {(view === 'tracks' || view === 'playlists') && (view === 'tracks' ? tracks.length > 0 : playlists.length > 0) && (
        <>
          <button 
            ...
            className={cn(
              "p-2 rounded-full transition-all paper-button",
              showSortMenu ? 'bg-apple-blue/10 text-apple-blue' : 'text-system-tertiary-label',
              settings.libraryControlsPosition === 'bottom' && cn(
                "flex-1 flex justify-center bg-secondary-system-background/50 h-11 items-center rounded-2xl paper-emboss",
                isMini && "h-9"
              )
            )}
          >
            <div className="flex items-center gap-2">
              <SortAsc size={isMini ? 18 : 20} />
              {settings.libraryControlsPosition === 'bottom' && <span className={cn("font-black uppercase tracking-widest italic font-serif", isMini ? "text-[9px]" : "text-[10px]")}>Sort</span>}
            </div>
          </button>
          <button 
            onClick={() => {
            ...
            className={cn(
              "font-bold px-4 rounded-full transition-all paper-button",
              isSelectMode ? 'bg-system-blue text-system-background' : 'text-system-blue',
              isMini ? 'text-[12px] py-1' : 'text-[13px] py-1.5',
              settings.libraryControlsPosition === 'bottom' && cn(
                "flex-1 flex justify-center bg-secondary-system-background/50 h-11 items-center rounded-2xl paper-emboss",
                isMini && "h-9"
              )
      ...
      {!isSelectMode && (
        <label className={cn(
          "flex items-center justify-center cursor-pointer active:scale-95 transition-all paper-button",
          settings.libraryControlsPosition === 'bottom' ? cn(
            "w-auto flex-1 bg-system-blue text-system-background h-11 rounded-2xl shadow-sm paper-emboss",
            isMini && "h-9"
          ) : cn(
            isMini ? "w-8 h-8 rounded-lg" : "w-10 h-10 rounded-full"
          )
        )}>
          <div className="flex items-center gap-2">
            <Plus size={isMini ? 18 : (settings.libraryControlsPosition === 'bottom' ? 20 : 24)} className={settings.libraryControlsPosition === 'bottom' ? "text-system-background" : "text-system-blue"} />
            {settings.libraryControlsPosition === 'bottom' && <span className={cn("font-black uppercase tracking-widest italic font-serif", isMini ? "text-[9px]" : "text-[10px]")}>Import</span>}
          </div>
  ...
  return (
    <div className={cn(
      "flex flex-col w-full max-w-7xl mx-auto px-4",
      isMini ? "pb-32" : "pb-40",
      settings.libraryControlsPosition === 'bottom' && (isMini ? "pt-4" : "pt-6")
    )}>
      <header className={cn(
        "flex flex-col bg-system-background gap-8",
        isMini ? "pb-4" : "pb-6"
      )}>
        <div className="flex justify-between items-center px-1">
          <h1 className={cn(
            "serif-title text-system-label flex items-center gap-3 italic",
            isMini ? "text-3xl" : "text-4xl"
          )}>
            Library
            {isSelectMode && selectedTrackIds.size > 0 && (
              <span className={cn("font-bold text-system-blue bg-system-blue/5 px-3 py-1 rounded-full animate-in fade-in zoom-in duration-300 paper-emboss", isMini ? "text-[12px]" : "text-sm")}>
        ...
        <div className={cn("flex px-1", isMini ? "gap-4" : "gap-6")}>
          <button 
            ...
            className={`serif-title transition-all paper-button ${isMini ? 'text-[18px]' : 'text-xl'} ${view === 'tracks' ? 'text-system-blue opacity-100 scale-105' : 'text-system-tertiary-label opacity-60 hover:opacity-100 italic'}`}
          >
            Tracks
          </button>
          <button 
            ...
            className={`serif-title transition-all paper-button ${isMini ? 'text-[18px]' : 'text-xl'} ${view === 'playlists' || view === 'playlist_detail' ? 'text-system-blue opacity-100 scale-105' : 'text-system-tertiary-label opacity-60 hover:opacity-100 italic'}`}
          >
            Playlists
          </button>
        </div>
      ...
      {showSortMenu && (view === 'tracks' || view === 'playlists') && !isSelectMode && (
        <div className={`paper-card flex flex-col animate-in fade-in slide-in-from-top-2 mx-2 mb-6 ${settings.appearance.uiMode === 'mini' ? 'p-3 gap-3' : 'p-4 gap-4'}`}>
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-bold uppercase tracking-widest text-system-secondary-label italic serif-title">Sort {view === 'tracks' ? 'Tracks' : 'Playlists'}</span>
            <div className="flex bg-secondary-system-background/50 rounded-xl p-1 paper-emboss">
              {(['recent', 'alphabetical', 'date'] as SortOption[]).map(s => (
                <button
                  key={s}
                  onClick={() => view === 'tracks' ? updateLibrarySettings({ sort: s }) : setPlaylistSort(s)}
                  className={`px-3 py-1 text-[10px] font-bold rounded-lg transition-all paper-button ${(view === 'tracks' ? settings.library.sort : playlistSort) === s ? 'bg-system-background shadow-sm text-system-blue' : 'text-system-secondary-label'}`}
                >
          ...
          {view === 'tracks' && (
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase tracking-widest text-system-secondary-label italic serif-title">Grouping</span>
                <div className="flex bg-secondary-system-background/50 rounded-xl p-1 overflow-x-auto no-scrollbar max-w-[70%] paper-emboss">
                  {(['none', 'alphabetical', 'numbers', 'minutes', 'day', 'week', 'month'] as GroupOption[]).map(g => (
                    <button
                      key={g}
                      onClick={() => updateLibrarySettings({ group: g })}
                      className={`px-3 py-1 text-[10px] font-bold rounded-lg transition-all whitespace-nowrap paper-button ${settings.library.group === g ? 'bg-system-background shadow-sm text-system-blue' : 'text-system-secondary-label'}`}
                    >
              ...
              <button 
                ...
                className="flex items-center justify-between px-2 paper-button"
              >
                <span className="text-[10px] font-bold uppercase tracking-widest text-system-secondary-label italic">Auto Group Recent Imports</span>
                <div className={`w-8 h-4 rounded-full relative transition-colors ${settings.library.groupByMinutes ? 'bg-system-blue' : 'bg-system-tertiary-label'}`}>
                  <motion.div className="absolute top-0.5 left-0.5 bg-system-background w-3 h-3 rounded-full" animate={{ x: settings.library.groupByMinutes ? 16 : 0 }} />
                </div>
              </button>
            </div>
          )}
            ...
            {groupedTracks.map((group, gIdx) => (
              <div key={gIdx} className="flex flex-col gap-2 relative">
                {group.label && (
                  ...
                  >
                    <h3 className="serif-title text-[14px] text-apple-text-secondary group-hover/header:text-apple-blue transition-colors px-2 italic">
                      {group.label}
                    </h3>
        ...
        {showImportTargetMenu && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-xl paper-card p-8 pb-12 shadow-2xl flex flex-col gap-6 overflow-hidden"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                   <h2 className="serif-title text-2xl italic tracking-tight">Import {pendingFiles.length} Tracks</h2>
                   <p className="text-[10px] text-system-tertiary-label font-bold uppercase tracking-widest mt-1 italic">Select Destination</p>
                </div>
                <button 
                  onClick={() => setShowImportTargetMenu(false)}
                  className="p-2 bg-secondary-system-background/50 rounded-full text-system-tertiary-label paper-button"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex flex-col gap-2 max-h-[50vh] overflow-y-auto no-scrollbar py-2">
                <button
                  ...
                  className="w-full flex items-center justify-between p-5 bg-secondary-system-background/50 hover:bg-secondary-system-background rounded-2xl transition-all group paper-button paper-emboss"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-system-blue/10 flex items-center justify-center text-system-blue transition-colors">
                      <Music size={20} />
                    </div>
                    <span className="serif-title text-[15px] italic">Library Only</span>
                  </div>
                  <ChevronRight size={18} className="opacity-30" />
                </button>

                <div className="h-[0.5px] bg-apple-border/30 my-2" />

                <button
                  onClick={() => processImport('new')}
                  className="w-full flex items-center justify-between p-5 bg-system-blue/5 hover:bg-system-blue/10 rounded-2xl transition-all group text-system-blue paper-button paper-emboss"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-system-blue/10 flex items-center justify-center transition-colors">
                      <Plus size={20} />
                    </div>
                    <span className="serif-title text-[15px] italic">Import into New Playlist</span>
                  </div>
                  <ChevronRight size={18} className="opacity-30" />
                </button>
                ...
                {playlists.map(p => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-5 bg-secondary-system-background/50 hover:bg-secondary-system-background rounded-2xl transition-all paper-button paper-emboss"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-gray-100/50 flex items-center justify-center text-system-tertiary-label">
                        <ListPlus size={20} />
                      </div>
                      <div className="flex flex-col items-start">
                        <span className="serif-title text-[15px] italic">{p.name}</span>
                        <span className="text-[10px] text-system-tertiary-label font-bold uppercase tracking-tight italic opacity-70">{p.trackIds.length} tracks</span>
                      </div>
                    </div>
                    <ChevronRight size={18} className="opacity-30" />
                  </button>
      ...
      <AnimatePresence>
        {isSelectMode && (
          <motion.div
            ...
            exit={{ y: 20, opacity: 0 }}
            className="fixed bottom-28 left-4 right-4 z-[100] flex flex-col gap-4 px-6 py-6 paper-card text-system-label"
          >
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-system-tertiary-label italic serif-title">{selectedTrackIds.size} Selected</span>
              <div className="flex gap-5">
                <button 
                  ...
                  className="text-[10px] font-bold text-system-blue uppercase tracking-[0.1em] paper-button italic serif-title"
                >
                ...
                <button 
                  ...
                  className="text-[10px] font-bold text-system-tertiary-label uppercase tracking-[0.1em] paper-button italic serif-title"
                >
                  Cancel
                </button>
              </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              <button 
                ...
                className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-system-fill hover:bg-system-fill/80 transition-colors disabled:opacity-20 paper-button paper-emboss"
              >
                <ListPlus size={20} />
                <span className="text-[9px] font-bold uppercase tracking-tight italic font-serif">{(editingPlaylistId || activePlaylistId) ? 'Copy' : 'Add'}</span>
              </button>
              ...
              {(editingPlaylistId || activePlaylistId) && (
                <button 
                  ...
                  disabled={selectedTrackIds.size === 0}
                  className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-system-fill hover:bg-system-pressed transition-colors paper-button paper-emboss"
                >
                  <Share size={20} />
                  <span className="text-[9px] font-bold uppercase tracking-tight italic font-serif">Move</span>
                </button>
              )}
              ...
              {(editingPlaylistId || activePlaylistId) ? (
                <button 
                  ...
                  disabled={selectedTrackIds.size === 0}
                  className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-system-red/20 text-system-red hover:bg-system-red/30 transition-colors paper-button paper-emboss"
                >
                  <Trash2 size={20} />
                  <span className="text-[9px] font-bold uppercase tracking-tight italic font-serif">Remove</span>
                </button>
              ) : (
                <button 
                ...
                >
                  <FolderPlus size={20} />
                  <span className="text-[9px] font-bold uppercase tracking-tight italic font-serif">New List</span>
                </button>
              )}
              ...
              <button 
                ...
                disabled={selectedTrackIds.size === 0}
                className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-system-fill hover:bg-system-fill/80 transition-colors text-system-red disabled:opacity-20 paper-button paper-emboss"
              >
                <Trash2 size={20} />
                <span className="text-[9px] font-bold uppercase tracking-tight italic font-serif">Delete</span>
              </button>
            </div>
            ...
            <AnimatePresence>
              {showBulkAddMenu && (
                <motion.div
                  ...
                  className="absolute bottom-full mb-6 left-0 right-0 paper-card p-6 text-system-label z-50 overflow-hidden"
                >
                  <div className="flex items-center justify-between mb-6 px-1">
                    <h3 className="serif-title text-xl italic tracking-tight">Select Playlist</h3>
                    <button onClick={() => setShowBulkAddMenu(false)} className="text-system-tertiary-label p-2 paper-button"><X size={20} /></button>
                  </div>
                  <div className="grid grid-cols-1 gap-2 max-h-64 overflow-y-auto no-scrollbar">
                    {playlists.map((p: any) => (
                      <button
                        ...
                        className="w-full text-left px-5 py-4 bg-secondary-system-background/50 hover:bg-secondary-system-background rounded-2xl flex items-center justify-between transition-all active:scale-[0.98] paper-button paper-emboss"
                      >
                        <span className="serif-title text-base italic">{p.name}</span>
                        <span className="text-[10px] text-system-tertiary-label font-bold uppercase tracking-widest">{p.trackIds.length} tracks</span>
                      </button>
                    ))}
                  </div>
                </motion.div>
...
const HighlightText = ({ text, highlight }: { text: string; highlight: string }) => {
  ...
  return (
    <>
      {parts.map((part, i) => 
        regex.test(part) ? (
          <span key={i} className="text-system-blue font-bold">{part}</span>
        ) : (
      ...
      <div className="flex items-center gap-4 px-4 h-[72px] leading-tight flex-shrink-0 border-b border-apple-border/30">
        {isSelectMode && (
          <button 
            ...
            className={`flex-shrink-0 transition-transform duration-200 ${isSelected ? 'scale-110 text-system-blue' : 'scale-100 text-system-tertiary-label'}`}
          >
        ...
        <button 
          ...
          className="flex-1 flex items-center gap-4 text-left min-w-0 h-full py-3 paper-button"
        >
          <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-secondary-system-background/50 flex items-center justify-center overflow-hidden relative paper-emboss border border-apple-border/50">
             <ArtworkImage src={track.artwork} className="w-full h-full opacity-90" iconSize={16} />
             {isActive && <div className="absolute inset-0 bg-system-blue/10 flex items-center justify-center"><div className="w-1.5 h-1.5 rounded-full bg-system-blue shadow-[0_0_8px_rgba(0,122,255,0.6)]" /></div>}
          </div>
          <div className="flex-1 min-w-0">
            <h4 className={`serif-title truncate text-[16px] ${isActive ? 'text-system-blue' : 'text-system-label'}`}>
              <HighlightText text={track.name} highlight={searchQuery} />
            </h4>
            <p className="text-[11px] text-system-secondary-label font-bold uppercase tracking-widest truncate mt-0.5 italic opacity-80">
          ...
          >
            <div className="px-14 py-4 flex flex-col gap-4">
               ...
               <button 
                  ...
                  className="flex items-center gap-3 text-[12px] font-semibold text-system-blue active:opacity-50"
               >
...
const PlaylistView = ({ playlists, onCreate, onDelete, onRename, tracks, onTrackPlay, isSelectMode, editingPlaylistId, selectedTrackIds, onToggleSelection, onEnterSelect, onOpen, searchQuery }: any) => {
  ...
  return (
    <div className={`flex flex-col gap-4 w-full max-w-7xl mx-auto pb-12`}>
      <button 
        onClick={onCreate}
        className={`w-full bg-secondary-system-background/50 border border-apple-border rounded-2xl flex items-center gap-4 transition-all active:scale-[0.98] paper-button paper-emboss ${isSelectMode ? 'hidden' : 'p-4'}`}
      >
        <div className="w-10 h-10 rounded-lg bg-system-blue/10 flex items-center justify-center text-system-blue">
          <Plus size={20} />
        </div>
        <span className="serif-title text-base italic">Create New Playlist</span>
      </button>
            ...
            <div key={playlist.id} className="group relative h-[88px] flex flex-col justify-center">
              <div className={`flex items-center gap-4 py-4 px-1 transition-opacity h-[87.5px] ${isSelectMode && !isEditingThis ? 'opacity-40' : 'opacity-100'}`}>
                  <button 
                    ...
                    className="flex-shrink-0 w-12 h-12 rounded-xl bg-secondary-system-background/50 flex items-center justify-center overflow-hidden border border-apple-border active:scale-95 transition-all paper-button paper-emboss"
                  >
                     {playlist.trackIds.length > 0 ? (
                       <ArtworkImage src={tracks.find((t: any) => t.id === playlist.trackIds[0])?.artwork} className="w-full h-full opacity-90" iconSize={20} />
                     ) : (
                ...
                <button 
                  onClick={() => !isSelectMode && onOpen(playlist.id)}
                  className="flex-1 min-w-0 text-left cursor-default paper-button px-2"
                >
                  <h3 className="serif-title text-[18px] text-system-label italic">
                    <HighlightText text={playlist.name} highlight={searchQuery} />
                  </h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    <p className="text-[11px] text-system-secondary-label font-bold uppercase tracking-widest italic opacity-70">{playlist.trackIds.length} tracks</p>
                    <span className="text-system-tertiary-label opacity-40">•</span>
                    <p className="text-[11px] text-system-secondary-label font-bold uppercase tracking-widest italic opacity-70">{formatTotalDuration(playlist.trackIds, tracks)}</p>
                  </div>
                </button>
                ...
                <div className="flex items-center gap-2">
                  {playlist.trackIds.length > 0 && (
                    <button 
                      ...
                      className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full transition-all paper-button ${isEditingThis ? 'bg-system-blue text-system-background' : 'text-system-blue bg-system-blue/5 hover:bg-system-blue/10 paper-emboss'}`}
                    >
                        ...
                        {isMenuOpen && (
                          <>
                            ...
                            <motion.div
                              ...
                              className="absolute right-0 top-full mt-2 w-48 paper-card z-50 py-2 overflow-hidden"
                            >
                              <button 
                                onClick={() => { 
                                ...
                                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-secondary-system-background/50 text-[13px] font-bold text-left text-system-label paper-button"
                              >
                              ...
                              <button 
                                ...
                                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-secondary-system-background/50 text-[13px] font-bold text-system-red text-left paper-button"
                              >
                                <Trash2 size={16} className="text-system-red" />
                                Delete
                              </button>
                            </motion.div>
                          </>
    ...
    <div className="flex flex-col min-h-full animate-in fade-in slide-in-from-right duration-300">
      <header className="bg-system-background flex flex-col gap-4 pt-4 pb-2">
        <div className="flex items-center justify-between px-2">
          <button onClick={onBack} className="p-2 text-system-blue font-bold flex items-center gap-1 active:opacity-50">
            <ArrowLeft size={20} />
          ...
          <div className="flex gap-1 relative">
                    ...
                    <>
                      ...
                      >
                        ...
                        <div className="px-4 py-2">
                          ...
                          {(['recent', 'alphabetical', 'date'] as SortOption[]).map(s => (
                            <button
                              ...
                              className={`w-full flex items-center justify-between py-1.5 text-[12px] font-bold ${sort === s ? 'text-system-blue' : 'text-system-secondary-label'}`}
                            >
                        ...
                        <button 
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 hover:bg-secondary-system-background text-[13px] font-bold text-system-red text-left"
                        >
                          <Trash2 size={16} className="text-system-red" />
                          Delete Playlist
                        </button>
            ...
            {isSelectMode ? (
              <button 
                ...
                className="text-system-blue text-[15px] font-bold px-3 active:opacity-50"
              >
                Done
              </button>
            ) : (
              <button onClick={() => onEnterSelect(true)} className="text-system-blue text-[15px] font-bold px-3 active:opacity-50">Select</button>
            )}
          </div>
        </div>
          ...
          return (
            <div key={track.id} ref={isActuallyPlaying ? activeTrackRef : null} className="group h-[64px] flex flex-col justify-center">
              <div className="flex items-center gap-3 py-3 px-1 leading-tight h-[63.5px]">
                ...
                >
                  <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-secondary-system-background flex items-center justify-center overflow-hidden relative">
                    <ArtworkImage src={track.artwork} className="w-full h-full" iconSize={16} />
                    {isActuallyPlaying && <div className="absolute inset-0 bg-system-blue/10 flex items-center justify-center"><div className="w-1.5 h-1.5 rounded-full bg-system-blue" /></div>}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className={`font-bold truncate text-[14px] ${isActuallyPlaying ? 'text-system-blue' : 'text-system-label'}`}>{track.name}</h4>
